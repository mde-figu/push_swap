...
/*   Updated: 2021/09/17 15:46:49 by mde-figu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../include/push_swap.h"

static int	loop(int half, t_lst **l_a, t_lst **l_b, t_vars *vars)
{
	int		q;
	int		go_rra;
...
void	re_push_b(t_lst **l_a, t_lst **l_b, t_vars *vars, int c)
{
	...
	vars->piv = get_middle(*l_b, (vars->parts_b)[0]);
	go_rrb = eval_rrr(*l_b, vars->piv, 1);
	while (i-- > 0 && half)
	{
	...
	repush_b_return(l_a, l_b, vars, c);
}
